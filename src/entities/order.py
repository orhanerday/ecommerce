...
class Order(Base):
    ...
    order_id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()), index=True)
    customer_id = Column(String(36), nullable=False)
    product_id = Column(String(36), nullable=False)
    price_paid = Column(Float, nullable=True)  # New field for price paid
    status = Column(SAEnum(OrderStatus, name="order_status"), default=OrderStatus.PENDING)
